...
window.window._jpHls = null; window.window._jpVideo = null;
function fmtTime(s) {
...
function initJplayer(v) {
  // 清理旧实例
  if (window._jpHls) { try { window._jpHls.destroy(); } catch(e){} window._jpHls = null; }
  const video = $('jpVideo');
  const jp = $('jp');
  window._jpVideo = video;
  ...
  speedMenu.querySelectorAll('.jp-menu-item').forEach(b => {
    b.onclick = (e) => {
      ...
      if (window._jpHls) {
        window._jpHls.config.maxBufferLength = Math.max(120, s * 120);
        window._jpHls.config.maxMaxBufferLength = Math.max(600, s * 600);
      }
  ...
  function buildQualityMenu(levels) {
    ...
    qualMenu.querySelectorAll('.jp-menu-item').forEach(b => {
      b.onclick = (e) => {
        ...
        if (window._jpHls) { window._jpHls.currentLevel = lvl; }
