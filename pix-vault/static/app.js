...
function applyTheme(theme) {
  if (theme && theme !== 'amber') document.documentElement.setAttribute('data-theme', theme);
  else document.documentElement.removeAttribute('data-theme');
  try { localStorage.setItem('pv_theme', theme || ''); } catch(e) {}
  document.querySelectorAll('.theme-swatch').forEach(el => {
    el.classList.toggle('active', (el.dataset.themeValue || '') === (theme || 'amber'));
  });
}
function loadTheme() {
  let saved = '';
...
function updatePosIndicator(img) {
  ...
  if (!el) return;
  if (!img) { el.classList.add('empty'); el.innerHTML = ''; return; }
  let mode = S.seqMode ? '顺序' : '随机';
  let cur = S.histIdx + 1;
  let total = Math.max(S.history.length, 1);
  if (S.seqMode && S.seqPool.length) { cur = Math.max(S.seqIdx + 1, 1); total = S.seqPool.length; }
  el.classList.remove('empty');
  el.innerHTML = `<span class="pos-mode">${mode}</span><span class="pos-cur">${cur}</span><span class="pos-sep">/</span><span class="pos-total">${total}</span>`;
}
...
function renderTagBar() {
  ...
  // 行尾"全选/全取消"
  const allTagsOn = S.allTags.length > 0 && S.allTags.every(t => S.activeTags.has(t));
  const btn = document.createElement('button');
  btn.className = 'bar-all-btn';
  btn.textContent = allTagsOn ? '全取消' : '全选';
...
function renderCatBar() {
  ...
  // 行尾"全选/全取消"
  const allCatsOn = S.categories.length > 0 && S.categories.every(c => S.activeCats.has(c.name));
  const btn = document.createElement('button');
  btn.className = 'bar-all-btn';
  btn.textContent = allCatsOn ? '全取消' : '全选';
  btn.addEventListener('click', () => {
...
function showImage(img) {
  ...
  const tryShow = () => {
    ...
    if (isMobileUI()) {
      if (pre.naturalWidth > pre.naturalHeight) el.classList.add('landscape-mobile');
      else el.classList.add('portrait-mobile');
    }
    updateInfo(img);
    showInfoBriefly();
    _preloadImg = null;
...
function toggleGallery() {
  ...
  if (S.isGallery) {
    ...
    $('#btn-color-group').style.display = 'none';
...
async function loadGallery() {
  ...
  showLoading(true);
  try {
    // 颜色分组：每个色桶均匀曝光（mode=balanced），非分组：普通随机
    const params = buildQuery().replace('?','&');
...
async function loadColorIndex() {
  if (S.colorIndex) return;
  try {
    const data = await api('/api/colors');
    S.colorIndex = data.index || {};
    S.colorBuckets = data.buckets || [];
  } catch(e) {
    S.colorIndex = {};
    S.colorBuckets = [];
  }
...
function renderGalleryByColor(grid, images) {
  // 按桶分组
  const groups = {};
  for (const img of images) {
    const ci = S.colorIndex[img.path];
    const bid = ci ? ci.bid : 'ungrouped';
    if (!groups[bid]) {
      const bucket = S.colorBuckets.find(b => b.id === bid) || {id:'ungrouped', name:'未分类', hex:'#555'};
      groups[bid] = {bucket, images: []};
    }
    groups[bid].images.push(img);
  }
  // 桶按"图像数量"降序
  ...
  for (const grp of ordered) {
    ...
    head.innerHTML = `<span class="cg-dot" style="background:${grp.bucket.hex}"></span>
      <span class="cg-name">${grp.bucket.name}</span>
      <span class="cg-count">${grp.images.length}</span>`;
    head.style.animationDelay = (gi * 0.015) + 's';
...
function toggleGroupByColor() {
  S.groupByColor = !S.groupByColor;
  $('#btn-color-group').classList.toggle('active', S.groupByColor);
  if (S.isGallery) loadGallery();
}
$('#btn-color-group').addEventListener('click', toggleGroupByColor);
...
function renderBrowseGrid() {
  ...
  BROWSE.scrollEl = $('#main-view');

  // 绑定事件（只绑一次，绑到 main-view 上因为现在外层滚）
  if (!mode._bound) {
    mode._bound = true;
    const mv = $('#main-view');
    mv.addEventListener('scroll', () => {
      if (!mv._rAF) {
        mv._rAF = requestAnimationFrame(() => {
          mv._rAF = null;
          paintBrowseRows();
        });
      }
...
function toggleBrowse() {
  ...
  $('#gallery-mode').style.display = 'none';
  $('#main-view').classList.remove('gallery');
  $('#main-view').classList.add('browse');
  $('#btn-mode').classList.remove('active');
...
function closeBrowse() {
  ...
  BROWSE.loaded = false;
  $('#browse-mode').style.display = 'none';
  $('#main-view').classList.remove('browse');
  $('#single-mode').style.display = '';
  $('#gallery-close').classList.remove('show');
...
document.addEventListener('keydown', e => {
  ...
  switch (e.key) {
    ...
    case 'Delete': case 'd': case 'D': e.preventDefault(); toggleDislike(); break;
...
$('#main-view').addEventListener('touchmove', e => {
  ...
  if ((S.isGallery || S.isBrowse) && dy > 0) {
    ...
    if (mv.scrollTop <= 5 && Math.abs(dy) > Math.abs(dx) * 1.5) {
      if (!touchHint) {
        touchHint = document.createElement('div');
        touchHint.className = 'pull-hint';
        document.body.appendChild(touchHint);
      }
      touchHint.textContent = '↻ 释放重新加载';
      touchHint.style.opacity = Math.min(0.9, dy / 100);
      touchHint.style.top = (10 + dy * 0.3) + 'px';
  ...
  if (!touchHint) {
    touchHint = document.createElement('div');
    touchHint.className = 'swipe-hint';
    document.body.appendChild(touchHint);
  }
  touchHint.textContent = dx > 0 ? '‹' : '›';
  touchHint.className = 'swipe-hint swipe-' + (dx > 0 ? 'left' : 'right');
...
// 文件名点击展开/收起（mobile 长文件名）
$('#info-name').addEventListener('click', e => {
  e.stopPropagation();
  $('#info-name').classList.toggle('expanded');
});
$('#info-name').addEventListener('touchend', e => {
  e.stopPropagation();
  // touch 设备上 click 不会触发（300ms 延迟），手动切换
  $('#info-name').classList.toggle('expanded');
}, { passive: true });
